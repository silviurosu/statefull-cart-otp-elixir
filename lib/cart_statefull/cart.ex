defmodule CartStatefull.Cart do
  ...
  def init([uuid]) do
    ...
    {:ok, %State{uuid: uuid, items: []}}
  end

  # HANDLERS for GenServer actions
  def handle_call({:list}, _from, %State{items: items} = state) do
    item_names = Enum.map(items, fn({_id, name}) -> name end)
    {:reply, %{buyer: state.buyer, items: item_names}, state, @default_timeout}
  end

  def handle_cast({:add_item, item}, %State{items: items} = state) do
    {:noreply, %State{state | items: [item | items]}, @default_timeout}
  end

  def handle_cast({:add_buyer, buyer}, state) do
    {:noreply, %State{state | buyer: buyer}, @default_timeout}
  end

  def handle_cast({:remove_item, item_id}, %State{items: items} = state) do
    new_items = Enum.reject(items, fn({id, _name}) -> item_id == id end)
    {:noreply, %State{state | items: new_items}, @default_timeout}
  end
